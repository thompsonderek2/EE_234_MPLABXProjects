...
#   ICxCON<15> = 1 (turn on input capture module)
#   ICxCON<9> = 1 (Capture rising edge first)
#   ICxCON<8> = 0 (16-bit timer resource capture)
#   ICxCON<7> = 1; ICTMR: Input Capture x Timer Select bit, 1 = TMR2 contents are captured on capture event
#   ICxCON<6:5> = 00; Interrupt on every capture event
#   ICxCON<2:0> = 001; Edge capture mode, every edge
    
LI $t0, 0b1000001010000001
SW $t0, IC3CONSET
